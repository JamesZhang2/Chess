...
class MaterialEvaluatorTest extends EvaluatorTest {
    ...
    void testMaterialEvaluation() throws IllegalBoardException, MalformedFENException {
        ...
        assertEquals(MaterialPartialEvaluator.QUEEN_VALUE, evaluator.evaluate(board2), DELTA);
        Board board3 = new BitmapBoard("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNB1KBNR w KQkq - 0 1");
        assertEquals(-MaterialPartialEvaluator.QUEEN_VALUE, evaluator.evaluate(board3), DELTA);
        Board board4 = new BitmapBoard("4k3/8/8/8/8/8/8/4K2R w - - 0 1");
        assertEquals(MaterialPartialEvaluator.ROOK_VALUE, evaluator.evaluate(board4), DELTA);
        Board board5 = new BitmapBoard("8/7P/7K/8/8/8/p7/Nk6 b - - 0 1");
        assertEquals(MaterialPartialEvaluator.KNIGHT_VALUE, evaluator.evaluate(board5), DELTA);
    }
}